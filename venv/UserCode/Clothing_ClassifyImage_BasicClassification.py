...
# Check the data of the trianing example
'''
train_images.shape
len(train_labels)
...
# Plt the test images for cheak data
'''
...
def plot_image(i, predictions_array, ture_label, img):
    ture_label, img = ture_label[i], img[i]
    plt.grid(False)
    plt.xticks([])
    plt.yticks([])
...
def plot_value_array(i,predictions_array, ture_label):
    ...
    thisplot[ture_label].set_color('blue')
...
plt.figure(figsize=(2*2*num_clos, 2*num_rows))
